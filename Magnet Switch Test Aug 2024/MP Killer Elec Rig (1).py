...
def encoderCalculation(startEncoder):
    return True
...
def main():
    ...
    #Test length variables
    cycles = 50000
    delayTime = 5 #seconds

    #Other variables
    encoderTolerance = 10000
    shouldCut = True
    cyclesBetweenCut = 2

    #Set script type
    
    logText("Starting Script " + " on " + str(SOM_NAME))

    #Encoder and distance tracking
    startEncoder = float(0)
    ...
    for i in range(cycles):
        ...
        for mp in mps:
            ...
            if(not i):
                #first cycle, unload without cut 
                #unloadWithoutCut(mp, 1000)    
                logText("first cycle")     

            #Log the MP being used
            mp_obj = getState()["data"]["liberty"]["data"]["vanguard"]["materialPods"][mp]
            assert mp_obj is not None
            serialNum = mp_obj["data"]["serialNumber"]
            logText('Testing MP - '+ str(serialNum) + " in slot " + str(mp)+ " cycle:" + str(i))

            #Check encoder reading at the start of the feed
            startEncoder = instantaneous_task.read_feed_encoder("feed",mp)   
            ...
            try:
                ...
                if(encoderDiff < (FEED_L- encoderTolerance)):
            ...
            if(bool(i%cyclesBetweenCut)):
                cycleCheck = False
                print("False", bool(i%cyclesBetweenCut))
            else:
                cycleCheck = True

            logText("Unloading with cut")
            if(unloadWithCut(mp, CUT_RETRACT_L)):
                logText("disconnect")
                errorStatus= True
                break

            # if(shouldCut & cycleCheck):
            #     #unload with a cut
            #     logText("Unloading with cut")
            #     if(unloadWithCut(mp, CUT_RETRACT_L)):
            #         logText("disconnect")
            ...
            #         logText("Current feed encoder "+ str(currentEncoder))
            #         logText("Difference in encoder " + str(encoderDiff))
            #         if(encoderDiff > encoderTolerance):
            #             logText("Something went wrong, potential quick disconnect. Encoder diff:" + str(encoderDiff) )
            #             errorStatus = True
            #     except:
            #         print("timeout, ignore")            
