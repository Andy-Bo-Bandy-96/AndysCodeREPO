...
def feedSetDistance(mp, distance) -> bool:
    ...
    if(readVanguardSensor(mp,"outgoing") & readVanguardSensor(mp,"ingoing")):
        ...
        logText("Filament not loaded correctly"+ DATE.strftime("%d_%m_%Y_%H_%M"))
        return True  
    ...
    for i in range(cycles):
        ...
        for mp in mps:
            ...
            try:
                ...
                logText(DATE.strftime("%d_%m_%Y_%H_%M"))
                logText("Difference in encoder " + str(encoderDiff))
                if(encoderDiff < (FEED_L- 10000)):
