...
liberty_unload = wrapLibertyDoJob(LibertyJob("unload", "liberty/unload", {"length": 0, "performCut": False, "useOutgoingSwitch": True, "checkLoaded": False}, "unloading"))
...
def main():
    ...
    CUT_L = 85 # 515mm, @700mm -> 600mm, @850mm->?
    for i in count(0):
        ...
        for mp in mps:
            ...
            # feed out a distance past the cutter to be cut
            time_consuming_task.mp_feed_filament_forward(mp, CUT_L).start_and_wait_til_done()
            ...
            time_consuming_task.mp_feed_filament_forward(mp, 36).start_and_wait_til_done() # 50mm? Idk
            ...
            while True:
                ...
                run_multiple_time_consuming_tasks_in_parallel(
                    ...
                    time_consuming_task.extrude(27.5, 500), # distance 10mm, rate 600
                    time_consuming_task.mp_feed_filament_forward(mp, 8),
                    # you can run more than 2 in parallel. as many as you want
                    # ...
                )
                odometer_value = instantaneous_task.read_printhead_encoder()
                if odometer_value >= 13:
                    break

            run_multiple_time_consuming_tasks_in_parallel(
                time_consuming_task.extrude_until_printhead_switch_is_released_and_then_extrude_one_more_time(150), # rate 300
